# Asset Entity Links Specification

## Purpose
Defines persistent, local-first cross-application references owned by LifeTrace Assets, including identity rules, offline mutation, synchronization, backup compatibility, and least-privilege rendering.

## Requirements

### Requirement: Persistent cross-app references
The Assets application MUST persist cross-application references as real EntityLink records rather than fabricated UI data.
...
Every Assets link MUST use `asset.asset` as its source entity type and MUST preserve the target entity type and target entity ID exactly.
...
Assets links MUST serialize to the existing LifeTrace Cloud `entity.link` contract and participate in normal Sync v1 snapshot, push, pull, acknowledgement, and optimistic conflict flows.
...
An existing Assets Cloud session that predates dedicated link scopes MUST continue syncing core asset entities instead of failing the entire sync cycle.
